...
public class DADCell : MonoBehaviour, IDropHandler
{
    ...
    public class DropEventDescriptor                                        // Info about item's drop event
    {
        ...
        public DADItem item;                                        // Dropped item
    ...
    private void OnAnyItemDragStart(DADItem item)
    {
        UpdateMyItem();
        if (myDadItem != null)
        {
            ...
            if (myDadItem == item)                                         	// If item dragged from this cell
            {
                ...
                switch (cellType)
                {
                    case CellType.DropOnly:
                        DADItem.icon.SetActive(false);              // Item can not be dragged. Hide icon
                        break;
                }
            }
        }
    ...
    /// </summary>
    /// <param name="item"> dragged item </param>
    private void OnAnyItemDragEnd(DADItem item)
    {
        UpdateMyItem();
        if (myDadItem != null)
        {
    ...
    public void OnDrop(PointerEventData data)
    {
        ...
        {
            DADItem item = DADItem.draggedItem;
            DADCell sourceCell = DADItem.sourceCell;
            if (DADItem.icon.activeSelf == true)                    // If icon inactive do not need to drop item into cell
            {
            ...
            if (item != null)
            {
                if (item.GetComponentInParent<DADCell>() == null)   // If item have no cell after drop
                {
    ...
    private void PlaceItem(DADItem item)
    {
        if (item != null)
        {
            DestroyItem();                                              // Remove current item from this cell
            myDadItem = null;
            DADCell cell = item.GetComponentInParent<DADCell>();
            if (cell != null)
            {
    ...
    public void AddItem(DADItem newItem)
    {
        if (newItem != null)
        {
            PlaceItem(newItem);
    ...
    public void SwapItems(DADCell firstCell, DADCell secondCell)
    {
        if ((firstCell != null) && (secondCell != null))
        {
            DADItem firstItem = firstCell.GetItem();                // Get item from first cell
            DADItem secondItem = secondCell.GetItem();              // Get item from second cell
                                                                            // Swap items
            if (firstItem != null)
            {
